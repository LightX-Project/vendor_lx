...
func init() {
	android.RegisterModuleType("lx_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = lxExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = lxExpandVariables(ctx, dep_file)
	...
	cmd := lxExpandVariables(ctx, String(g.properties.Cmd))
